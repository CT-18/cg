...
def redrawClosest(point, ax1, color='black', look='--', lookPoint='o'):
    for i in range(len(point)):
        drawPoint(point[i], ax1, color, lookPoint)
        drawLine(point[i - 1], point[i], ax1, color, look)


def drawIntersection(p3, fig):
...
def drawPoint(point, ax1, color='red', look='.'):
    q1, = ax1.plot(point[0], point[1], look, color=color)
    q.insert(-1, q1)


def drawLine(pointStart, pointEnd, ax1, color='red', look='-'):
...
redrawClosest(p1, ax1, 'blue', '-', '.')
redrawClosest(p2, ax1, 'green', '-', '.')
...
# ax1.add_patch(PolygonPatch(p3))
redrawClosest(p3Res, ax1, 'grey', '.', 'o')
ax1.set_xlim(MIN_X - 1, MAX_X + 1)
ax1.set_ylim(MIN_Y - 1, MAX_X + 1)
...
def redrawClosest(point, ax1, color='black', look='--'):
    points = np.array(point)
    for i in range(len(points)):
        drawPoint(point[i], ax1, color)
        drawLine(point[i - 1], point[i], ax1, color, look)
...
def test(f, n=10):
    for i in range(0, n):
        ...
        if size_y < 0:
            raise Exception('MAX_Y must be moer than MIN_Y')
        p1 = generateFigure(MIN_X + size_x / 2, MIN_Y + size_y / 2, min(size_x, size_y) / 2.5, 0.35, 0.2,
                            COL_POINTS_FIRST)
        p2 = generateFigure(MIN_X + size_x / 2, MIN_Y + size_y / 2, min(size_x, size_y) / 2.5, 0.35, 0.2,
                            COL_POINTS_SECOND)
        redrawClosest(p1, ax1, 'black', '-')
        redrawClosest(p2, ax1, 'red', '-')
        p1 = getPolygon(p1)
        p2 = getPolygon(p2)
        result = overlaying(p1, p2)
        answer = f(p1, p2)
        if answer == result:
            continue
        else:
